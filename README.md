...
When querying http://localhost:8101/posts?_limit=10 we get a list of this sort of objects:
...
And when querying http://localhost:8101/users we get this sort of objects:
...
- http://localhost:8100 - your GraphQL-server
- http://localhost:8101 - your REST-server
...
Look at a post object in http://localhost:8101/posts and add the remaining fields. You can skip `user` for now, we will return to that later.
...
The endpoint you want to query is the following: http://localhost:8101/posts?_limit=2
...
We are able to fetch several users at once by calling [`/users?id=user1&id=user2&..`](http://localhost:8101/users/?id=user1&id=user2).
